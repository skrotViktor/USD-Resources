...
HairProcHairProceduralSceneIndexRefPtr HairProcHairProceduralSceneIndex::New(
    const HdSceneIndexBaseRefPtr &inputSceneIndex) {
    return TfCreateRefPtr(
        new HairProcHairProceduralSceneIndex(inputSceneIndex));
}

HairProcHairProceduralSceneIndex::HairProcHairProceduralSceneIndex(
    const HdSceneIndexBaseRefPtr &inputSceneIndex)
...
HairProcHairProceduralSceneIndex::GetPrim(const SdfPath &primPath) const {
    ...
    if (prim.primType == HdPrimTypeTokens->basisCurves) {
        ...
        HdBasisCurvesSchema curveSchema =
        ...
        HairProcHairProceduralSchema hairProcSchema =
            HairProcHairProceduralSchema::GetFromParent(prim.dataSource);

        if (curveSchema && primvarSchema && hairProcSchema) {
            if (auto deformer = _deformerMap.find(primPath);
                deformer != _deformerMap.end()) {
                prim.dataSource = _HairProcDataSource::New(
...
SdfPathVector HairProcHairProceduralSceneIndex::GetChildPrimPaths(
    const SdfPath &primPath) const {
    return _GetInputSceneIndex()->GetChildPrimPaths(primPath);
}

void HairProcHairProceduralSceneIndex::_PrimsAdded(
    ...
    for (const HdSceneIndexObserver::AddedPrimEntry &entry : entries) {
        if (entry.primType == HdPrimTypeTokens->basisCurves) {
            ...
            if (curveSchema && primvarSchema && hairProcSchema) {
                _init_deformer(entry.primPath, hairProcSchema, curveSchema,
                               primvarSchema);
            }
        }
    }
...
void HairProcHairProceduralSceneIndex::_PrimsRemoved(
    const HdSceneIndexBase &sender,
...
void HairProcHairProceduralSceneIndex::_PrimsDirtied(
    const HdSceneIndexBase &sender,
    const HdSceneIndexObserver::DirtiedPrimEntries &entries) {

    // If any prims in entries are part of _targets, we need to also dirty their
    // sources, ie the hairProcedural prims
    if (!_IsObserved()) {
        return;
    }
...
void HairProcHairProceduralSceneIndex::_init_deformer(
    const SdfPath &primPath, HairProcHairProceduralSchema &procSchema,
    HdBasisCurvesSchema &basisCurvesSchema, HdPrimvarsSchema &primvarSchema) {
    ...
    HairProcHairProceduralDeformerSharedPtr deformer =
        std::make_shared<HairProcHairProceduralDeformer>(targetDs, sourceDs,
                                                         primPath);
